...
if __name__ == '__main__':
    ...
    files = [join(args.dir, f) for f in files]
    img_files = [f for f in files if f.endswith("_pred.npy")]
    depth_files = [f for f in files if f.endswith("_depth.npy")]
    ...
    for i in range(len(depth_files)):
        ...
        statistics, n = measure(img, depth, args)
        n_total += n
        print(f"Step: {i} n: {n}")
        for k in statistics.keys():
            print(f"Step: {i} Statistics[{k}]: {statistics[k]}")
            if k not in statistics_total:
                statistics_total[k] = 0
            statistics_total[k] += statistics[k] * n
