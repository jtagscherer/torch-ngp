    ...
    def train_step(self, data, patch_data=None):
        ...
        if self.global_step > style_training_start_step:
            ...
            if 'images' in data:
                ...
                ground_truth = gt_rgb
                ground_truth = ground_truth.reshape(patch_H, patch_W, 3)[np.repeat(inx_h, len(inx_w)), np.tile(inx_w, len(inx_h)),
                ...
                if self.opt.output_debug_images and self.global_step < style_training_start_step + 50:
                    ...
                    full_patch_image = full_patch_image.reshape(patch_H, patch_W, 3).permute(2, 0, 1).contiguous()
                    torch_vis_2d(full_patch_image)
                    plt.savefig(f'/tmp/nerfout/{self.global_step}_full_patch.png')

                    full_gt_image = full_gt.detach()
                    full_gt_image = full_gt_image.reshape(patch_H, patch_W, 3).permute(2, 0, 1).contiguous()
